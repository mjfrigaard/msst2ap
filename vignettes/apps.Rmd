...
Three of the applications in the `dbsap` package come from the [Modules section of Mastering Shiny](https://mastering-shiny.org/scaling-modules.html), with a few adjustments: 
...
Printing the output from `reactiveValuesToList()` in the top-level of the standalone function shows the namespaces and modules throughout the application. 
...
-   **`selectDataVarApp()` nests the `dataset` and `selectVar` modules *inside* the `selectDataVarUI()` and `selectDataVarServer()` functions:**
    -   Inside `selectDataVarUI()`:
        ...
        - **`selectVarInput()`:**
            -   [x] `selectVarInput()` also displays the variables in the `sidebarPanel()` from the returned `data` object in `datasetServer()`
